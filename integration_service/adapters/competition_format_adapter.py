...
class CompetitionFormatAdapter:
    ...
    async def create_competition_format(self, token: str, request_body: dict) -> str:
        ...
        async with ClientSession() as session, session.post(url, headers=headers, json=request_body) as resp:
            res = resp.status
    ...
    def get_default_competition_format(self, format_type: str) -> dict:
        ...
        config_files_directory = f"{Path.cwd()}/integration_service/config"
        ...
        try:
            ...
            logging.exception(f"Current directory {Path.cwd()} - content {os.listdir()}")
    ...
    async def update_competition_format(self, token: str, request_body: dict) -> str:
        ...
        async with ClientSession() as session, session.put(url, headers=headers, json=request_body) as resp:
            res = resp.status
